...
- **GLSL LSP — 추가 확장**: Phase 24 / 25 / 26 가 진단·심볼 테이블·Hover·시맨틱 토큰까지 다룸. **Go-to-definition (F12 / Cmd+Click)**, **Find references**, **Rename refactor** 는 별도 단계. `core/glsl/symbolTable.ts` 가 이미 line/column 을 담고 있어 goto 구현은 작은 추가 PR 로 가능 — 사용자 요청 시 진행. CodeMirror 6 유지(Monaco 전환은 여전히 회피 권장).
