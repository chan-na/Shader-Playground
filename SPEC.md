...
- **비범위 (Out of scope)**: 노드 카드 변경 — 실제 링크 실패는 이미 `ErrorBadge`가 커버하므로 카드에 사전 경고 배지를 추가하지 않는다(디자인 요청서 대상). 배열 크기 불일치(`out vec3 v[2]` vs `in vec3 v[3]`), 보간 한정자 불일치(`flat`/`smooth` 교차)는 의도적 비탐지 — `varyingContract.ts` 모듈 주석 참조. A-3(Shader 노드를 vertex/fragment 두 노드로 분리)은 별도 결정(D-5) 대기 중.
