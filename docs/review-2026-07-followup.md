...
### 고치기 전에 재현하기 — probe 요령

F3·F21·F22에서 **"코드 판독으로 확실해 보여도 먼저 런타임에서 실행해 본다"**가 매번 값을 했다. F22는 판독으로는 완결된 연쇄였지만 실제로 돌려 보고서야 스토어가 58자→56자로 갈아치워지는 것까지 확인됐고, F21은 실제로 1×1 PNG가 떨어지는 것을 보고서야 처방 범위가 확정됐다.

- **임시 probe 테스트는 `src/**/*.test.ts(x)` 위치여야 vitest가 잡는다.** 다 쓰면 반드시 지울 것 — Knip은 테스트 파일을 entry로 보므로 남겨두면 조용히 통과한다.
- **vitest 설정이 `console.log`를 삼킨다.** probe 값을 보려면 `node:fs`로 파일에 쓴다. E2E probe는 반대로 `throw new Error(...)`로 값을 실패 메시지에 실어 보내는 편이 빠르다.
- **"아무 일도 안 일어남" 류 단언은 그 단언 자체가 빨간지 따로 본다.** 앞선 단언에서 먼저 죽으면 헛단언이 통과로 위장된다.
- **되돌리기 요령**: `cp <file> temp/<name>.bak` → `perl -0pi -e 's/…//'`로 한 조각만 뒤집기 → 테스트 → `cp`로 복원. 층이 여러 개면 **한 층씩** 뒤집어 각 층이 독립적으로 red인지 확인한다(F21에서 스토어 가드와 툴바 분기를 따로 검증했다).
- **신규 E2E는 `--repeat-each=8` 단독 실행으로 안정성을 실증한다.** 단독에서 플레이크인데 파일 전체로 돌리면 숨는 사례가 있었다(아래 2번).
- **E2E 전체 실행에서 `browserContext.newPage` 타임아웃이 나면 인프라 플레이크를 의심한다** — 앱 코드가 돌기 전 셋업 단계라 변경과 무관하다. 해당 스펙만 재실행해 확인할 것. 또한 `npm run test:e2e | tail`의 exit code는 `tail`의 것이므로 **파이프로 판정하지 말 것**.
