...
export async function loadGltfFromArrayBuffer(
  ...
  const reshape = {
    attributes: {
      POSITION: attrs.POSITION && attrs.POSITION.value
        ? { value: attrs.POSITION.value }
        : undefined,
      NORMAL: attrs.NORMAL && attrs.NORMAL.value
        ? { value: attrs.NORMAL.value }
        : undefined,
      TEXCOORD_0: attrs.TEXCOORD_0 && attrs.TEXCOORD_0.value
        ? { value: attrs.TEXCOORD_0.value }
        : undefined,
    },
    indices: first.indices && first.indices.value
      ? { value: first.indices.value }
      : undefined,
  };
