...
function pickAttribute(
  attrs: Record<string, { value: ArrayLike<number>; size?: number } | undefined>,
  keys: string[],
) {
  for (const k of keys) {
    const a = attrs[k];
...
export async function loadObjFromText(text: string, name = 'obj'): Promise<GeometryHandle> {
...
export function toGeometryHandle(
  ...
  const pos = pickAttribute(parsed.attributes, ['POSITION', 'positions', 'position']);
  if (!pos) {
    throw new Error('OBJ has no POSITION attribute');
  }
  const norm = pickAttribute(parsed.attributes, ['NORMAL', 'normals', 'normal']);
  const uv = pickAttribute(parsed.attributes, ['TEXCOORD_0', 'TEXCOORD', 'uvs', 'uv']);
  ...
  const uvs = uv ? new Float32Array(uv.value) : new Float32Array(vertexCount * 2);
...
function computeFlatNormals(
  ...
  for (let t = 0; t < triCount; t++) {
    ...
    const [ax, ay, az] = [positions[i0 * 3], positions[i0 * 3 + 1], positions[i0 * 3 + 2]];
    const [bx, by, bz] = [positions[i1 * 3], positions[i1 * 3 + 1], positions[i1 * 3 + 2]];
    const [cx, cy, cz] = [positions[i2 * 3], positions[i2 * 3 + 1], positions[i2 * 3 + 2]];
