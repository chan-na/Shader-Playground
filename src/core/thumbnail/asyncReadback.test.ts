...
function makeGL() {
  ...
  const fakeFB = { fbo: {} as WebGLFramebuffer, color: { texture: {} } } as never;
...
function fakeFB(width: number, height: number) {
  return { fbo: {} as WebGLFramebuffer, color: { texture: {} as WebGLTexture, width, height }, width, height, depth: null } as never;
}
