...
import { deserializeProject, serializeProject, PROJECT_FORMAT_VERSION } from './serialization';
import { createChainDemoGraph, CHAIN_DEMO_LAYOUT, createDemoGraph, DEMO_LAYOUT } from './demoGraph';
...
describe('serializeProject / deserializeProject', () => {
  ...
  it('rejects payload with wrong format tag', () => {
    expect(() => deserializeProject({ format: 'other', version: 1, graph: { nodes: [], edges: [] } })).toThrow();
  });

  it('rejects payload missing graph', () => {
    expect(() => deserializeProject({ format: 'shader-playground', version: 1 })).toThrow();
  });
  ...
  it('attaches a warning for future format versions but still loads', () => {
    ...
    expect(restored.warnings.some((w) => w.includes('newer than supported'))).toBe(true);
