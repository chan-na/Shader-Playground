...
import { encodeShareUrl, decodeShareHash } from './shareUrl';
...
const graph: Graph = {
  ...
  edges: [
    { id: 'e1', source: 'mesh1', sourceHandle: 'mesh', target: 'shader1', targetHandle: 'mesh' },
    { id: 'e2', source: 'shader1', sourceHandle: 'texture', target: 'output1', targetHandle: 'texture' },
  ],
};
...
describe('shareUrl', () => {
  it('round-trips a project through encode/decode', async () => {
    ...
    expect(decoded!.graph.nodes.map((n) => n.id)).toEqual(['mesh1', 'shader1', 'output1']);
