      ...
      {node.paramKind === 'float' && (
        <div className="inspector-row">
          ...
          <input
            ...
            onChange={(e) => setParamValue(node.id, parseFloat(e.target.value) || 0)}
          />
        </div>
      )}
        ...
        <>
          {['x', 'y', 'z'].map((axis, i) => (
            <div className="inspector-row" key={axis}>
              <span style={{ width: 12, color: '#888', fontFamily: 'monospace' }}>{axis}</span>
              <input
                type="range"
                min={-1}
                max={1}
      ...
      {node.paramKind === 'color' && Array.isArray(node.value) && (
        <div className="inspector-row">
          <input
            ...
            onChange={(e) => setParamValue(node.id, [...hexToRgb(e.target.value)])}
          />
          <span style={{ color: '#888', fontFamily: 'monospace', fontSize: 11 }}>
            {(node.value as number[]).slice(0, 3).map((x) => x.toFixed(2)).join(', ')}
          </span>
        </div>
      )}
        ...
        <>
          ...
          {['Scale', 'Offset'].map((label, i) => (
            <div className="inspector-row" key={label}>
              <span style={{ width: 48, color: '#888', fontSize: 11 }}>{label}</span>
              <input
                type="range"
                min={i === 0 ? -5 : -10}
                max={i === 0 ? 5 : 10}
                ...
                onChange={(e) => {
                  const cur = Array.isArray(node.value) ? node.value.slice() : [1, 0];
              ...
              <input
                type="number"
                step={0.01}
                value={(Array.isArray(node.value) ? node.value[i] : i === 0 ? 1 : 0)?.toFixed(3) ?? '0.000'}
                onChange={(e) => {
                  const cur = Array.isArray(node.value) ? node.value.slice() : [1, 0];
